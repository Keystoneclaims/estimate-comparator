...
def _guess_room_from_line(text: str, current_room: str) -> str:
    ...
    # Opening/dimension lines are not room headers. Keep the current repair area.
    if re.match(r"^(door|window|missing wall)\b", s, re.I):
        return current_room
    ...
    if not clean or clean == "room" or len(clean) > 55:
        return current_room
    ...
    with pdfplumber.open(io.BytesIO(content)) as pdf:
        for page in pdf.pages:
            ...
            for raw_line in text.splitlines():
                ...
                # Xactimate often lists small subrooms (for example Closet (2)) inside a larger
                # repair section. For comparison/reporting, keep line items assigned to the
                # parent repair area unless no parent has been detected yet.
                if re.match(r"^Subroom:\s*", clean_line, re.I):
                    if not current_room:
