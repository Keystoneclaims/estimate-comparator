...
def reason_for_missing_item(line: EstimateLine) -> str:
    """Create a practical, scope-based justification for an item in our estimate that is absent from the opposing estimate."""
    text = (f"{line.category} {line.description}" or "").lower()
    text = re.sub(r"[^a-z0-9/& ]+", " ", text)
...
def carrier_language_for_missing_item(line: EstimateLine) -> str:
    ...
    return (
        f"The opposing estimate does not appear to include {line.description} in {room}. "
    ...
    for company in company_lines:
        ...
        if best_idx is None or best_score < threshold:
            rows.append(ComparisonRow(
                ...
                note="Your estimate contains this item, but no strong opposing estimate match was found.",
            ))
            continue
    ...
    for idx, carrier in enumerate(carrier_lines):
        if idx not in used_carrier:
            rows.append(ComparisonRow(
                ...
                note="Opposing estimate contains this item, but it was not found in your estimate.",
            ))
